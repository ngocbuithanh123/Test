...
function _initPaylines(){
    ...
    _aPaylineCombo[5] = [{ row: 1, col: 0 }, { row: 0, col: 1 }, { row: 1, col: 2 }, { row: 0, col: 3 }, { row: 1, col: 4 }];
    _aPaylineCombo[6] = [{ row: 1, col: 0 }, { row: 2, col: 1 }, { row: 1, col: 2 }, { row: 2, col: 3 }, { row: 1, col: 4 }];
...
function _initSymbolsOccurence(){
    _aRandSymbols = new Array();
        ...
        //OCCURENCE FOR SYMBOL FREESPIN
        for(i=0;i<5;i++){
            _aRandSymbols.push(12);
        }

        //OCCURENCE FOR SYMBOL FREESPIN
        for(i=0;i<5;i++){
            _aRandSymbols.push(13);
        }
...
function _initSymbolWin(){
    ...
    _aSymbolWin[11] = [0,0,0,0,0,0];
    _aSymbolWin[12] = [0,0,0,0,0,0];
...
function generLosingPattern(){
    ...
    for(var i=0;i<NUM_ROWS;i++){
        ...
        }while(_aRandSymbols[iRandIndex] === 12 || _aRandSymbols[iRandIndex] === 13 || _aRandSymbols[iRandIndex] === 11);
            ...
            if(j == 0){
                ...
                do{
                    ...
                    iRandSymbol = _aRandSymbols[iRandIndex];
                        ...
                        iRandSymbol === 11 || iRandSymbol === 12 || iRandSymbol === 13);
...
function generateRandomSymbols(bFreespin,bBonus){
    for(var i=0;i<NUM_ROWS;i++){
        ...
        for(var j=0;j<NUM_REELS;j++){
            do{
                ...
                iRandSymbol = _aRandSymbols[iRandIndex];
                _aFinalSymbols[i][j] = iRandSymbol;
            }while(iRandSymbol === 12 || iRandSymbol === 13);
        }
    }
    ...
    if(bFreespin === 1){
        ...
        for(var k=0;k<_iNumSymbolFreeSpin;k++){
            ...
            _aFinalSymbols[iRandRow][aCurReel[k]] = 13;
        }
    }else if(bBonus === 1){
        ...
        for(var k=0;k<iNumBonusSymbol;k++){
            ...
            _aFinalSymbols[iRandRow][aCurReel[k]] = 12;
        }
    }
}
    ...
    function checkWin(bFreespin,bBonus,iNumBettingLines){
        ...
        if(bFreespin === 1){
            ...
            for(var i=0;i<NUM_ROWS;i++){
                for(var j=0;j<NUM_REELS;j++){
                    if(_aFinalSymbols[i][j] === 13){
                        aCellList.push({row:i,col:j,value:13});
                    }
                }
            }

            _aWinningLine.push({line:0,amount:0,num_win:aCellList.length,value:13,list:aCellList});
            
        }else if(bBonus === 1){
            var aCellList = new Array();
            for(var i=0;i<NUM_ROWS;i++){
                for(j=0;j<NUM_REELS;j++){
                    if(_aFinalSymbols[i][j] === 12){
                        aCellList.push({row:i,col:j,value:12});
                    }
                }
            }

            _aWinningLine.push({line:0,amount:0,num_win:aCellList.length,value:12,list:aCellList});
	}
