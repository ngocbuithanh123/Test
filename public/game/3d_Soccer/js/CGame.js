...
function CGame(oData){
    ...
    this._init = function(){
        ...
        var oData = {   // image to use
            images: [s_oSpriteLibrary.getSprite('logo_new')], 
        ...
        _oFreespinPanel = new CFreespinPanel(s_oAttachSection);

        this._initStaticSymbols();

        var oSpriteSheet = new createjs.SpriteSheet(oData);
        _oLogoNew = createSprite(oSpriteSheet, "normal",165,0,230,86);
        _oLogoNew.x = CANVAS_WIDTH/2 + 35;
        _oLogoNew.y = 10;
        _oLogoNew.stop();
        s_oAttachSection.addChild(_oLogoNew);
    ...
    this.getSymbol = function(){
        ...
        for(var i=0; i< _aFinalSymbolCombo.length;i++){
            for(var j=0; j < _aFinalSymbolCombo[i].length;j++){
                ...
                if(_aFinalSymbolCombo[i][j] === 12){
                    //bonus
                    _oIndexBonusArray.push(j);
					_iCountBonusSymbol++;
                }
                if(_aFinalSymbolCombo[i][j] === 13){
                    //freespin
    ...
    this._endReelAnimation = function(){
        ...
        if( !(_oInterface.CheckFreeSpinPanel() === false && _iTotFreeSpin === 0)){
        ...
        if(_aWinningLine.length > 0){
            ...
            if(_oLogoNew.currentAnimation === "freespin" && _iTotFreeSpin === 0){
                _oLogoNew.gotoAndStop("normal");
            }
          
            if(_iTotFreeSpin > 0){
                _oLogo.visible = false;
                _oLogoFreeSpin.visible = true;
                _oInterface.SetVisibleFreeSpinPanel(true);
            }else{
                _oLogo.visible = true;
                _oLogoFreeSpin.visible = false;
                _oInterface.SetVisibleFreeSpinPanel(false);
        ...
        if(_oProfile.GetSweepstakesPoints() < fee && _iTotFreeSpin === 0){
		 ...
		 if(_iBonus === BONUS_WHEEL){
            _oCurSymbolWinSound = playSound("bonus_win",1,0);
            _oPopup.showPopup(3);
        ...
        if(isStartFreeSpin === true){
            ...
            _oPopup.showPopupText(0, " ",2);
            _oLogoNew.gotoAndPlay("freespin");
        }
        ...
        if(_aWinningLine.length > 0){
            ...
            if(_iTotFreeSpin > 0){
                ...
                if(_oLogoNew.currentAnimation === "freespin"){
                    _oLogoNew.gotoAndStop("normal");
                }
