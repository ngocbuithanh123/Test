...
var _iNextColToStop = 0;
var _iCurReelLoops = 0;
...
function CGame(oData){
    ...
    this.holdStop = function(){
        _iNextColToStop++;
        _iNumReelsStopped += 2;
        console.log("_iNextColToStop: " + _iNextColToStop);
        if(_iNextColToStop > 4){
            this._endReelAnimation();
        }
    };
    ...
    this._endReelAnimation = function(){
        ...
        if(_oProfile.GetSweepstakesPoints() < fee && _iTotFreeSpin === 0){
		 ...
		 if(_iBonus === BONUS_WHEEL){
            _oCurSymbolWinSound = playSound("BigWin",1,0);
            _oPopup.showPopup(3);
