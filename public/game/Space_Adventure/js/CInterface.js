...
function CInterface(iCurBet,iTotBet,iMoney){
	...
	var _ListHoldBtn = new Array();
	var _ListIsHold = new Array();
	...
	this.InitHoldBtn = function(){
		
		var oSprite = s_oSpriteLibrary.getSprite('HoldBtn')
        _oHoldBtn1 = new CHoldToggle(408,285,oSprite,_oIsHold1,s_oAttachSection);
        _oHoldBtn1.addEventListener(ON_MOUSE_UP, this.onHold1, this);
		_oHoldBtn1.setIsHold(true);
		
		oSprite = s_oSpriteLibrary.getSprite('HoldBtn')
        _oHoldBtn2 = new CHoldToggle(535,285,oSprite,_oIsHold2,s_oAttachSection);
        _oHoldBtn2.addEventListener(ON_MOUSE_UP, this.onHold2, this);
		_oHoldBtn2.setIsHold(true);
		
		oSprite = s_oSpriteLibrary.getSprite('HoldBtn')
        _oHoldBtn3 = new CHoldToggle(660,285,oSprite,_oIsHold3,s_oAttachSection);
        _oHoldBtn3.addEventListener(ON_MOUSE_UP, this.onHold3, this);
		_oHoldBtn3.setIsHold(true);
		
		oSprite = s_oSpriteLibrary.getSprite('HoldBtn')
        _oHoldBtn4 = new CHoldToggle(786,285,oSprite,_oIsHold4,s_oAttachSection);
        _oHoldBtn4.addEventListener(ON_MOUSE_UP, this.onHold4, this);
		_oHoldBtn4.setIsHold(true);
		
		oSprite = s_oSpriteLibrary.getSprite('HoldBtn')
        _oHoldBtn5 = new CHoldToggle(912,285,oSprite,_oIsHold5,s_oAttachSection);
        _oHoldBtn5.addEventListener(ON_MOUSE_UP, this.onHold5, this);
		_oHoldBtn5.setIsHold(true);
		
		_ListHoldBtn.push(_oHoldBtn1,_oHoldBtn2,_oHoldBtn3,_oHoldBtn4,_oHoldBtn5);
		_ListIsHold.push(_oIsHold1,_oIsHold2,_oIsHold3,_oIsHold4,_oIsHold5)
	};
    ...
    this.InitTopButton = function(){
        ...
        _oInfoBut = new CTextButton(316 + (oSprite.width / 2), 44, 0, 3, oSprite, TEXT_PAYTABLE," ", FONT_REGULAR, "#ffffff", 17);
        _oInfoBut.addEventListener(ON_MOUSE_UP, this._onInfo, this);

        oSprite = s_oSpriteLibrary.getSprite('info_but');
        _oPreviewBut = new CTextButton(439 + (oSprite.width / 2), 44, 0, 3, oSprite, TEXT_PREVIEW," ", FONT_REGULAR, "#ffffff", 17);
        _oPreviewBut.addEventListener(ON_MOUSE_UP, this._onPreview, this);

        oSprite = s_oSpriteLibrary.getSprite('info_but');
        _oQuitBut = new CTextButton(930 + (oSprite.width / 2), 44, 0, 3, oSprite, "QUIT"," ", FONT_REGULAR, "#ffffff", 17);
    ...
    this.InitBottom1 = function(){
        ...
        _oMinusLineBut = new CTextButton(261 + (oSprite.width / 2), 544, 0, 0,oSprite," "," ",FONT_GAME,"#ffffff",30);
        _oMinusLineBut.addEventListener(ON_MOUSE_UP, this._onMinusLine, this);

        oSprite = s_oSpriteLibrary.getSprite('PlusBtn');
        _oPlusLineBut = new CTextButton(318 + (oSprite.width / 2), 544, 0, 0, oSprite, " "," ", FONT_GAME, "#ffffff", 30);
        _oPlusLineBut.addEventListener(ON_MOUSE_UP, this._onPlusLine, this);
        
        oSprite = s_oSpriteLibrary.getSprite('MinusBtn');
        _oMinusCoinBut = new CTextButton(391 + (oSprite.width / 2), 544, 0, 0, oSprite, " "," ",FONT_GAME,"#ffffff",30);
        _oMinusCoinBut.addEventListener(ON_MOUSE_UP, this._onMinusBet, this);

        oSprite = s_oSpriteLibrary.getSprite('PlusBtn');
        _oPlusCoinBut = new CTextButton(461 + (oSprite.width / 2), 544, 0, 0, oSprite, " "," ", FONT_GAME, "#ffffff", 30);
        _oPlusCoinBut.addEventListener(ON_MOUSE_UP, this._onPlusBet, this);

        oSprite = s_oSpriteLibrary.getSprite('but_maxbet_bg');
        _oMaxBetBut = new CTextButton(528 + (oSprite.width / 2), 545, 9, 1,oSprite,TEXT_MAX_BET," ",FONT_REGULAR,"#ffffff",16);
        _oMaxBetBut.addEventListener(ON_MOUSE_UP, this._onMaxBet, this);

        oSprite = s_oSpriteLibrary.getSprite('but_autospin');
        _oAutoSpinBut = new CTextButton(675 + (oSprite.width / 2), 545, -7, 3, oSprite, TEXT_AUTOSPIN," ", FONT_REGULAR, "#ffffff", 14); //732
        _oAutoSpinBut.addEventListener(ON_MOUSE_UP, this._onAutoSpin, this);

        oSprite = s_oSpriteLibrary.getSprite('spin_but');
        _oSpinBut = new CTextButton(610 + (oSprite.width / 2), 530, -1, -4, oSprite, " "," ", FONT_ZDYK, "#ffffff", 37);
        _oSpinBut.addEventListener(ON_MOUSE_UP, this._onSpin, this);

        _oNumLinesText = new createjs.Text(NUM_PAYLINES, "20px " + FONT_REGULAR, "#ffffff");
        ...
        _oCoinText = new createjs.Text(iCurBet.toFixed(2), "20px " + FONT_REGULAR, "#ffffff");
        _oCoinText.x =  452;
        _oCoinText.y = 551;
        ...
        _oWinText = new createjs.Text(TEXT_CURRENCY + "0.00", "20px " + FONT_REGULAR, "#ffffff");
        _oWinText.x = 860;
        _oWinText.y = 551;
        ...
        _oMoneyText = new createjs.Text(TEXT_CURRENCY + iMoney.toFixed(2) , "20px " + FONT_REGULAR, "#ffffff");
        _oMoneyText.x = 992;
        _oMoneyText.y = 550;
        ...
        _oBetTitText = new createjs.Text(TEXT_BET, "17px " + FONT_BOLD, "#ffffff");
        _oBetTitText.x = 452;
        _oBetTitText.y = 522;
        ...
        _oWinTitText = new createjs.Text("WIN", "17px " + FONT_BOLD, "#ffffff");
        _oWinTitText.x = 860;
        _oWinTitText.y = 522;
        ...
        _oWinningTitText = new createjs.Text("WINNINGS", "17px " + FONT_BOLD, "#ffffff");
    ...
    this.InitBottom2 = function(){
        ...
        _oRedeemBut = new CTextButton(670 + (oSprite.width / 2), 612, -2, 3, oSprite, "REDEEM"," ", FONT_REGULAR, "#ffffff", 17);
        _oRedeemBut.addEventListener(ON_MOUSE_UP, this._onRedeem, this);

        oSprite = s_oSpriteLibrary.getSprite('Btn2');
        _oAddEntriesBut = new CTextButton(825 + (oSprite.width / 2), 612, -1, 3, oSprite, "ADD ENTRIES"," ", FONT_REGULAR, "#ffffff", 14);
        _oAddEntriesBut.addEventListener(ON_MOUSE_UP, this._onAddEntries, this);

        _oEntriesTitText = new createjs.Text("ENTRIES", "17px " + FONT_BOLD, "#ffffff");
        _oEntriesTitText.x = 579;
        _oEntriesTitText.y = 584;
        _oEntriesTitText.textAlign = "center";
        _oEntriesTitText.textBaseline = "alphabetic";
        s_oAttachSection.addChild(_oEntriesTitText);

        _oEntriesText = new createjs.Text("50000", "20px " + FONT_REGULAR, "#ffffff");
        _oEntriesText.x =  579;
        _oEntriesText.y = 614;
        ...
        _oNextTitText = new createjs.Text("NEXT", "17px " + FONT_BOLD, "#ffffff");
        _oNextTitText.x =  397;
        _oNextTitText.y = 584;
        ...
        _oNextResultText = new createjs.Text("$0.00", "20px " + FONT_REGULAR, "#ffffff");
        _oNextResultText.x =  420;
        _oNextResultText.y = 614;
        ...
        _oOnNextResultToggle = new CToggle(446,578,oSprite,s_bAudioActive,s_oAttachSection);
    ...
    this.enableGuiButtons = function(){
        ...
        _oRedeemBut.enable();
        _oQuitBut.enable();
		this.setVisibleHoldButton(true);
    };
    ...
    this.disableGuiButtons = function(bAutoSpin){
        ...
        _oBanner2But.disable();
				this.setVisibleHoldButton(false);
    };
    ...
    this._onMaxBet = function(){
		...
		this.onHold = function(index, isHold){
		if(isHold){
			reelHoldIndex = index;
			reelHoldResult = this._getReelResult(index);
			if(index === 0){
				_iNextColToStop = 1;
				_iCurReelLoops = 1;
				_iNumReelsStopped = 2;
			...
			reelHoldIndex = -1;
            reelHoldResult = new Array(-1,-1,-1);
			_iNextColToStop = 0;
			_iCurReelLoops = 0;
			_iNumReelsStopped = 0;
		}
	...
	this.HideAllHold = function(){
        for(var i=0; i< _ListHoldBtn.length;i++){
            _ListHoldBtn[i].ChangeImage(false);
        }
    };
	
	this.onHold1 = function(){
		_oIsHold1 = !_oIsHold1;
		var index = 0;
		this.onHold(index, _oIsHold1);
	};
	
	this.onHold2 = function(){
		_oIsHold2 = !_oIsHold2;
		var index = 1;
		this.onHold(index, _oIsHold2);
	};
	
	this.onHold3 = function(){
		_oIsHold3 = !_oIsHold3;
		var index = 2;
		this.onHold(index, _oIsHold3);
	};
	
	this.onHold4 = function(){
		_oIsHold4 = !_oIsHold4;
		var index = 3;
		this.onHold(index, _oIsHold4);
	};
	
	this.onHold5 = function(){
		_oIsHold5 = !_oIsHold5;
		var index = 4;
		this.onHold(index, _oIsHold5);
	};
	
	this.setVisibleHoldButton = function(visible){
		for(var i=0; i< _ListHoldBtn.length;i++){
			if(i === reelHoldIndex){
				continue;
			}
			_ListHoldBtn[i].setVisible(visible);
		}
		_isClickHold = visible;
	};
	...
	this._getReelResult = function(index){
        var reelSymbol = new Array();
        var gameSymbol = new Array();
        if(_aFinalSymbols !== null) 
        {
            gameSymbol = _aFinalSymbols;
        }else{
            gameSymbol = _aFinalSymbolCombo;
        }
        for(var i=0;i<NUM_ROWS;i++){
            reelSymbol.push(gameSymbol[i][index]);
        }
        return reelSymbol;
    };
