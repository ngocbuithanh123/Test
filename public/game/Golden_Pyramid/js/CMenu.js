function CMenu(){
    ...
    this._init = function(){
        ...
        _oButPlay = new CTextButton((CANVAS_WIDTH / 2), CANVAS_HEIGHT - 164, 0, 0,oSprite,TEXT_SPIN," ",FONT_ZDYK,"#ffffff",45);
        _oButPlay.addEventListener(ON_MOUSE_UP, this._onButPlayRelease, this);

        if(DISABLE_SOUND_MOBILE === false || s_bMobile === false){
            var oSprite = s_oSpriteLibrary.getSprite('audio_icon');
            _pStartPosAudio = {x: CANVAS_WIDTH - (oSprite.width/4) - 10, y: (oSprite.height/2) + 5000};   
        ...
        if(SHOW_CREDITS){
            ...
            _pStartPosCredits = {x:(oSprite.height/2) + 10,y:(oSprite.height/2) + 10 + 5000};
            ...
            _pStartPosFullscreen = {x: _pStartPosCredits.x + oSprite.width,y:_pStartPosCredits.y};
             ...
             _pStartPosFullscreen = {x:(oSprite.height/2) + 10,y:(oSprite.height/2) + 10 + 5000};
        }
        ...
        if (_fRequestFullScreen && inIframe() === false){
            ...
            _oButFullscreen = new CToggle(_pStartPosFullscreen.x,_pStartPosFullscreen.y + 5000,oSprite,s_bFullscreen,s_oAttachSection);
