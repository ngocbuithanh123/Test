...
function CGame(oData){
    ...
    this._init = function(){
        ...
        _oFreeSpinPanel.x = 800;
        _oFreeSpinPanel.y = 13;
		...
		_oBonusPanelChicken = new CBonusPanelChicken();
        _bUpdate = true;
        _oMenu = new CMenus();
		//_oBonusPanelChicken.show(5,0.1, 1, 5);
        _oBgBonusPopup = createBitmap(s_oSpriteLibrary.getSprite('bg_loading_bonus'));
		_oBgBonusPopup.x = 0;
        _oBgBonusPopup.y = 0;
        _oBgBonusPopup.alpha = 0;
        s_oAttachSection.addChild(_oBgBonusPopup);
    };
    ...
    this._initLogoAnim = function(){
        //show Logo
        var oData = {   // image to use
            images: [s_oSpriteLibrary.getSprite('logoAnim')], 
            // width, height & registration point of each sprite
            frames: {width: 250, height: 59,regX:125,regY:0}, 
            animations: {  normal: 0,freespin:[1,27,"stop_freespin"],stop_freespin:27 }
        };

        _pStartPosLogo = {x:660,y:35};
        var oSpriteSheet = new createjs.SpriteSheet(oData);
        _oLogoAnim = createSprite(oSpriteSheet, "normal",125,0,250,59);
        _oLogoAnim.x = _pStartPosLogo.x;
        _oLogoAnim.y = _pStartPosLogo.y;
        s_oAttachSection.addChild(_oLogoAnim);
    ...
    this._endReelAnimation = function(){
        ...
        if(isStartFreeSpin === true){
            _oCurSymbolWinSound = playSound("MegaWin",1,0);
            _oPopup.showPopup(4);
            _oPopup.showPopupText(0, " ",2);
            _oLogoAnim.gotoAndPlay("freespin");
        }
    ...
    this.fadeInSymbolInWin = function(){
        ...
        setTimeout(function(){s_oGame._showWin();},iTime);
    };
    ...
    this._showWin = function(){
        ...
        if(_iCurWinShown === _aWinningLine.length){
            ...
            if(_iTotFreeSpin > 0){
                ...
                //_oBonusPanel.show(_iCurBonusPrizeIndex, _iCurBet);
                createjs.Tween.get(_oBgBonusPopup).to({alpha:1}, 1000).call(function(){
                    s_oGame.hideBonusPopup();
                });
            ...
            var iEndX = iStartX + 0  ;
            var iStartY = 0 + (aList[0].row * SYMBOL_HEIGHT);
            ...
            for(var k=1;k<aList.length+1;k++){
                ...
                if(k<aList.length){
                    ...
                    iEndX = (aList[k].col*(SYMBOL_WIDTH+SPACE_BETWEEN_SYMBOLS)) + 0;
                    iStartY = iEndY//SYMBOL_HEIGHT/2 + (aList[k].row * SYMBOL_HEIGHT);
                    iEndY =  0 + (aList[k].row * SYMBOL_HEIGHT) + Math.floor(Math.random() * 21) -10;
                }
            }
    ...
    this.hideBonusPopup = function(){
        setTimeout(() => {
            s_oGame.ShowBonusPanel();
            createjs.Tween.get(_oBgBonusPopup).to({alpha:0}, 1000).call(function(){});
        }, 1000);
    };

    this.ShowBonusPanel = function(){
        _oBonusPanelChicken.show(5,_iCurBet, _iCurBonusPrizeIndex, _iCountBonusSymbol);
        _iCurState = GAME_STATE_BONUS;
    };

    this._drawNextCutLine = function(){
        var iTime = 50;
        console.log("_iCurPointToDraw: " + _iCurPointToDraw);
        console.log("_aShapesToDraw.length: " + _aShapesToDraw.length);
        if(_iCurPointToDraw < _aShapesToDraw.length){
            ...
            if(_iCurPointToDraw > 0 && _iCurWinShown > 0){
                ...
                if(_iCurPointToDraw%2!==0){
                    ...
                    _aAnimSymbol[iRow][iCol].playAnim(_aFinalSymbolCombo[iRow][iCol],(_aWinningLine[_iCurWinShown-1].amount * _iCurBet),_iCurPointToDraw >= (_aShapesToDraw.length-2)?true:false);                    
                } 
            }
            _iCurPointToDraw++;
        }
